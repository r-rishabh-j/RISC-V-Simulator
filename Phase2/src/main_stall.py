...
#     print("Wrong knob")
#     sys.exit()
RunSim_stall.memory.InitMemory(MachineCodeParser.PC_INST, MachineCodeParser.DATA)
RunSim_stall.RunSim(reg_print=print_reg_file, buffprint=print_buff_file)
...
with open(f"RegisterDump_stall.mc", "w") as fileReg:
    for i in range(32): # for all 32 registers
        fileReg.write(f"x{i} ")  # print address of register for eg. x5
        if (RunSim_stall.registers.reg[i] >= 0):
            fileReg.write(padhexa(hex(RunSim_stall.registers.reg[i])).upper().replace('X', 'x'))
        else:
            reg = RunSim_stall.registers.reg[i] & 0xffffffff  # signed
            fileReg.write(hex(reg).upper().replace('X', 'x'))
        fileReg.write("\n")

#dumping memory
with open(f"MemoryDump_stall.mc", "w") as fileMem:  # input is dictionary with key as address and value as data
    lst = []  # stores keys present in dictionary
    temp_lst = []  # stores base address
    for key in RunSim_stall.memory.data_module.memory:
        lst.append(key)
    lst.sort()
    for x in lst:
    ...
    for i in temp_lst:
        ...
        if i in lst:
            fileMem.write(f"{(padhexa(hex(RunSim_stall.memory.data_module.memory[i])).upper())[8:]} " )  # if key in dictionary, print its data
        else:
            fileMem.write("00 ")  # if key not in dictionary, print 00
        if (i + 1) in lst:
            fileMem.write(f"{(padhexa(hex(RunSim_stall.memory.data_module.memory[i + 1])).upper())[8:]} ")
        else:
            fileMem.write("00 ")
        if (i + 2) in lst:
            fileMem.write(f"{(padhexa(hex(RunSim_stall.memory.data_module.memory[i + 2])).upper())[8:]} ")
        else:
            fileMem.write("00 ")
        if (i + 3) in lst:
            fileMem.write(f"{(padhexa(hex(RunSim_stall.memory.data_module.memory[i + 3])).upper())[8:]} ")
        else:
            fileMem.write("00  ")
        fileMem.write("\n")  # new line
    lst = []  # stores keys present in dictionary
    temp_lst = []
    for key in RunSim_stall.memory.text_module.memory:
        lst.append(key)
    lst.sort()
    for x in lst:
        temp = x - (x % 4)  # storing base address in temp
    ...
    for i in temp_lst:
        fileMem.write(f"{(padhexa(hex(i)).upper().replace('X', 'x'))} ")  # printing base address
        if i in lst:
            fileMem.write(f"{(padhexa(hex(RunSim_stall.memory.text_module.memory[i])).upper())[8:]} " )  # if key in dictionary, print its data
        else:
            fileMem.write("00 ")  # if key not in dictionary, print 00
        if (i + 1) in lst:
            fileMem.write(f"{(padhexa(hex(RunSim_stall.memory.text_module.memory[i + 1])).upper())[8:]} ")
        else:
            fileMem.write("00 ")
        if (i + 2) in lst:
            fileMem.write(f"{(padhexa(hex(RunSim_stall.memory.text_module.memory[i + 2])).upper())[8:]} ")
        else:
            fileMem.write("00 ")
        if (i + 3) in lst:
            fileMem.write(f"{(padhexa(hex(RunSim_stall.memory.text_module.memory[i + 3])).upper())[8:]} ")
        else:
            fileMem.write("00  ")
        fileMem.write("\n")  # new line
