...
#program load
RiscSim.memory.InitMemory(MachineCodeParser.PC_INST, MachineCodeParser.DATA)
#Run the simulator
RiscSim.RunSim()
...
with open(f"MemoryDump_non_pipelined.mc", "w") as fileMem:  # input is dictionary with key as address and value as data
    ...
    for key in RiscSim.memory.data_module.memory:
        lst.append(key)
    lst.sort()
    for x in lst:
    ...
    for i in temp_lst:
        ...
        if i in lst:
            fileMem.write(f"{(padhexa(hex(RiscSim.memory.data_module.memory[i])).upper())[8:]} " )  # if key in dictionary, print its data
        else:
            fileMem.write("00 ")  # if key not in dictionary, print 00
        if (i + 1) in lst:
            fileMem.write(f"{(padhexa(hex(RiscSim.memory.data_module.memory[i + 1])).upper())[8:]} ")
        else:
            fileMem.write("00 ")
        if (i + 2) in lst:
            fileMem.write(f"{(padhexa(hex(RiscSim.memory.data_module.memory[i + 2])).upper())[8:]} ")
        else:
            fileMem.write("00 ")
        if (i + 3) in lst:
            fileMem.write(f"{(padhexa(hex(RiscSim.memory.data_module.memory[i + 3])).upper())[8:]} ")
        else:
            fileMem.write("00  ")
        fileMem.write("\n")  # new line
    lst = []  # stores keys present in dictionary
    temp_lst = []  # stores base address
    for key in RiscSim.memory.text_module.memory:
        lst.append(key)
    lst.sort()
    for x in lst:
        temp = x - (x % 4)  # storing base address in temp
    ...
    for i in temp_lst:
        fileMem.write(f"{(padhexa(hex(i)).upper().replace('X', 'x'))} ")  # printing base address
        if i in lst:
            fileMem.write(f"{(padhexa(hex(RiscSim.memory.text_module.memory[i])).upper())[8:]} " )  # if key in dictionary, print its data
        else:
            fileMem.write("00 ")  # if key not in dictionary, print 00
        if (i + 1) in lst:
            fileMem.write(f"{(padhexa(hex(RiscSim.memory.text_module.memory[i + 1])).upper())[8:]} ")
        else:
            fileMem.write("00 ")
        if (i + 2) in lst:
            fileMem.write(f"{(padhexa(hex(RiscSim.memory.text_module.memory[i + 2])).upper())[8:]} ")
        else:
            fileMem.write("00 ")
        if (i + 3) in lst:
            fileMem.write(f"{(padhexa(hex(RiscSim.memory.text_module.memory[i + 3])).upper())[8:]} ")
        else:
            fileMem.write("00  ")
        fileMem.write("\n")  # new line
